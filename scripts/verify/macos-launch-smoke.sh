#!/usr/bin/env bash
# scripts/verify/macos-launch-smoke.sh — UI gate: the packaged macos-emitter.app
...
APP="$WDR_DIST_DIR/macos/macos-emitter.app"
...
sleep 4
if ! pgrep -x macos-emitter >/dev/null; then
    die "GUI process 'macos-emitter' not running after launch (bug: opens with no UI?)"
fi
log "process 'macos-emitter' is alive after launch"
...
if "$CHECKBIN" "macos-emitter" "Wavelink"; then
...
# --- 4) clean quit --------------------------------------------------------------
osascript -e 'tell application "macos-emitter" to quit' 2>/dev/null || true
sleep 1
if pgrep -x macos-emitter >/dev/null; then
