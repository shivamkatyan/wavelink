...
| **rubato** | Bounded adaptive resampling on receiver (ADR-007 drift correction). Alt: sample-slip insertion/deletion (audible), emitter-side clock slaving (API-gated), no-op (buffer grows) | https://github.com/HEnquist/rubato | MIT/Apache-2.0, static | 5.0.0 (2026-08) | Active (Henquist) | None critical; Polyphase/FFT resampler CPU + quality measured in ADR-005/007 bench | All 5 (pure Rust; simd optional is Rust, no native) | API choices (Sinc/Tukey) settled; upstream has property tests; our drift tests inject ±ppm | Pure Rust; small | **direct** (behind jitter/drift module) | Pin 5.0.0; update: major bumps require re-run of drift-slew bound tests |
| **mdns-sd** | mDNS browse/advertise for discovery (ADR-006). Alt: zeroconf (license ambiguity, rejected), libmdns, manual IP/QR (also shipped as fallback) | https://github.com/keepsimple1/mdns-sd | Apache-2.0/MIT | 0.21.2 (2026-09-05 — very recent) | Active (commits 2026) | None critical; no async runtime forced in crate (it includes its own select loop; integrate into our tokio executor — B0 integration item) | All 5; pure Rust; iOS local-network permission path validated (PLATFORM_MATRIX) | Stable-ish 0.x minor churn; upstream unit tests exist; our discovery adapter tests + mDNS saturation tests (THREAT_MODEL #6/#8) | Pure Rust, small; per-interface socket behaviour to validate on each OS | **direct** (wrapped behind `Discovery` adapter) | Pin 0.21.2; update policy: every minor re-run discovery adapter + iOS/Windows path tests |
