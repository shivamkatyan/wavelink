...
|---|---|---|---|---|---|
| R01 | QUIC CUBIC CWND collapse under 1% loss starves audio datagrams | Transport | P1/B0 | BBR evaluation in netem spike; redundancy/FEC; UDP fallback. **Spike resolved (ADR-003): keep CUBIC + redundancy/UDP fallback.** | resolved (ADR-003) |
| R02 | Lossless frames don't fit datagram MTU (incompressible worst case) | Audio/Codec | P1/B0 | ADR-005: reliable stream for lossless; small FLAC blocks; frame-size CI calculator. **Measured + resolved (ADR-003/005): lossless rides the reliable stream; noise@512=2144 B fits 4 Ki.** | resolved |
...
| R11 | Clock-drift estimator oscillation | Audio | P1/B0 | WLS + outlier rejection + deadband + slew bound; **implemented 2026-09-14 (WS-B): `wdr_refsim::drift` estimator + synthetic drift-injection tests + `bit_exact` honesty.** | resolved (WS-B) |
...
| R16 | RT callback alloc/lock/blocking regression | Audio | B2/B6 | RT_CONTRACT.md allowed-op whitelist; wdr_rt no-alloc/no-lock SPSC; **rt-guard abort-on-alloc feature IMPLEMENTED (2026-09-13/14: `#[global_allocator]` abort + `panic="abort"` + `#![no_std]` wdr_rt + zero-alloc deny-alloc probe + `StallDetector` watchdog + subprocess probes)**; CI stress+watchdog; native-runner RT evidence gate (WSL2 cannot substitute); hardware-lab worst-case validation | implemented; native-runner RT evidence still gated |
