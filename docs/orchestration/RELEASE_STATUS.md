...
Snapshot date: **2026-09-13 (0.0.1 genesis)** · Fresh single-commit history in the `wavelink` repo, carrying forward the previously verified state (229 tests / 0 fail; the v0.2.0 release of the retired `wireless-hifi-relay` repo is history) · Honest, evidence-backed state per Phase; no simulated claim passed off as physical.
...
## Soak status
- 60-minute clean soak (lossless FLAC, real-time paced): **PASS** on the source WSL2 host (B1-SOAK-EVIDENCE.md: 67-min wall, lossless hash preserved, 0 underruns/fatal, RSS flat 1.59–3.76 MiB / 107 samples).
- macOS-host: harness fixed for hermetic Linux binaries (host binaries are Mach-O; in-image binaries now used). Referencesystem e2e through the netem bridge to be re-validated on this host with a shortened soak; the full 60-min clean soak on macOS = queued scheduled job.
...
|---|---|---|
| **macOS desktop Receiver render path** | New feature: render the QUIC stream on macOS via AVAudioEngine / HAL output. The macOS window already carries the staged **Receiver** role selector — selecting it shows an honest "Desktop receiver render is the next milestone" alert today (`app/main.swift` `roleChanged`). Needs a Receiver render seam + a TCC/USB-DAC session to validate | 🔒 feature not started (UI staged only) |
...
| 11 | Discovery/pairing/encryption/revocation/malformed/redaction pass security tests | ✅/🟡 | crypto/session/telemetry tests green (Noise XX, replay, key-sep, AEAD tamper, redaction poison); mDNS discovery not yet wired through a shell |
